...
public:
     InputSimulator() { timeBeginPeriod(1); }
    ~InputSimulator() { timeEndPeriod(1); }

    void SetTargetWindow(HWND hwnd) { targetWindow = hwnd; }
    ...
    void SimulateKey(WORD vkCode, bool down) {
        ...
        if (vkCode == VK_CONTROL || vkCode == VK_SHIFT ||
            vkCode == VK_MENU    || vkCode == VK_UP    ||
            vkCode == VK_DOWN    || vkCode == VK_LEFT  ||
            vkCode == VK_RIGHT   || (vkCode >= VK_F1 && vkCode <= VK_F24))

            { lParam |= (1 << 24); }

        if (down) { PostMessage(targetWindow, WM_KEYDOWN, vkCode, lParam); }
        else      { PostMessage(targetWindow, WM_KEYUP, vkCode, lParam); }
    ...
    void PreciseSleep(DWORD ms) {
        ...
        if (ms >= 2) { Sleep(ms); } else {

            LARGE_INTEGER freq, start, end;
            QueryPerformanceFrequency(&freq);
            QueryPerformanceCounter(&start);
            end.QuadPart = start.QuadPart + (freq.QuadPart * ms / 1000);
            while (start.QuadPart < end.QuadPart) { QueryPerformanceCounter(&start); }
